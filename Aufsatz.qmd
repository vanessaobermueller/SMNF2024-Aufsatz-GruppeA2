...
Insgesamt haben 125 Personen an der Online-Studie teilgenommen. Die dabei für die Analyse zu berücksichtigen Daten beinhalten die von 119 Teilnehmenden (*n =* 119). Zu den Personen, die herausgefiltert wurden, gehören eine Person, welche unter 18 Jahren ist und deswegen nicht berücksichtigt werden kann. Zudem fünf weitere Datensätze, die einer Testgruppe angehören. 
...
ergebnissurvey <- ergebnissurvey %>% mutate(HCT_E = ifelse(first_variant == "eva-ai", HCT1T, HCT2T ))
ergebnissurvey <- ergebnissurvey %>% mutate(HCT_R = ifelse(first_variant == "rd-ai", HCT1T, HCT2T ))


deskriptive_statistik <- function(data, scale_name) {
  n <- nrow(ergebnissurvey)
  alpha <- round(psych::alpha(data %>% select(starts_with(scale_name)), check.keys = TRUE)$total$raw_alpha,2)
  mean_val <- round(mean(data[[scale_name]], na.rm = TRUE),2)
  median_val <- round(median(data[[scale_name]], na.rm = TRUE),2)
  min_val <- round(min(data[[scale_name]], na.rm = TRUE),2)
  max_val <- round(max(data[[scale_name]], na.rm = TRUE),2)
  sd_val <- round(sd(data[[scale_name]], na.rm = TRUE),2)
...
Grundsätzlich wollten wir untersuchen, ob es einen Zusammenhang zwischen der Technikaffinität, also dem ATI-Score und dem Alter der Probanden gibt. 
...
Desweiteren wollten untersuchen, ob es bei der Präzision der richtigen Entscheidungen altersbezogene signifikante Unterschiede gibt. 
...
ggplot(ergebnissurvey, aes(x = ergebnissurvey$age , y = ergebnissurvey$HCT_R)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE) +
  labs(title = "Korrelation zwischen Alter und HCT-Score der Recommender AI",
       x = "Alter",
       y = "HCT-Score")
