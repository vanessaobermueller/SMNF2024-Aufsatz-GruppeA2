...
**GitHub Repository:** https:\\github.com:vanessaobermueller/SMNF2024-Aufsatz-GruppeA2.git **Fuer die Abgabe aktueller GitHub Hash:** 8b25396
...
Die gewählte Methode für die Onlineumfrage ist die Verwendung von Skalen. Dabei werden verschiedene Skalenniveaus angewendet, um unterschiedliche Arten von Daten zu erfassen. Nominale Daten lassen sich nicht in eine Rangfolge bringen. Dies bedeutet, dass wir nicht sagen können, ob ein Wert besser oder schlechter als ein anderer ist, sondern lediglich, ob es einen Unterschied gibt. Nominale Daten geben uns Informationen über die Häufigkeit bestimmter Merkmale. In unserer Umfrage wird das nominale Skalenniveau für das Geschlecht der Teilnehmenden verwendet, wobei die Kategorien "Männlich" und "Weiblich" erfasst werden. Ordinalskalierte Daten können wir in eine Rangfolge bringen, jedoch können wir keine genauen Aussagen über die Abstände zwischen den Werten machen. Wir können feststellen, welche Werte höher oder niedriger sind, aber nicht, wie viel höher oder niedriger sie sind. Ordinale Daten geben uns Informationen über die Häufigkeit und die Rangfolge bestimmter Merkmale.

In unserer Umfrage werden der höchste allgemeinbildenden Schulabschluss und der höchste berufliche Abschluss auf ordinaler Basis erfasst, da sie eine Reihenfolge haben, aber keine genauen Abstände zwischen den Abschlüssen angegeben sind. Für den schulischen Abschluss stehen folgende Optionen zur Auswahl: "(noch) keinen Abschluss", "Hauptschulabschluss", "Realschulabschluss", "Abitur". Für den beruflichen Abschluss stehen folgende Optionen zur Auswahl: "(noch) keinen Abschluss", "Lehre/Ausbildung", "Fachschule", "Universität/Hochschule". Für metrische Daten können wir eine Rangfolge erstellen und die Abstände zwischen den Werten sind klar definiert. Metrische Daten ermöglichen uns, sowohl die Häufigkeit als auch die Rangfolge und den genauen Abstand zwischen den Werten zu bestimmen.
...
#| message: false
#| warning: false
#| fig-cap: "Zusammenhang zwischen Personen unter und über dem Durchschnittsalter und dem gemessenen ATI-Score "
#| label: fig-durchschnittati
...
plot_data <- data.frame(
  Gruppe = c("Jünger als Durchschnitt", "Älter als Durchschnitt"),
  ATI_Mean = c(mean_ati_young, mean_ati_old)
)

# Barplot erstellen
ggplot(plot_data, aes(x = Gruppe, y = ATI_Mean, fill = Gruppe)) +
  geom_bar(stat = "identity") +
  labs(
       x = "",
       y = "Durchschnittlicher ATI-Score") +
  theme_minimal() +
  scale_fill_brewer(palette = "Set2")

```

@fig-durchschnittati zeigt, dass Jüngere Teilnehmer im Schnitt einen leicht höheren ATI Score von `r round(mean_ati_young,2)` als ältere Teilnehmer besitzen, welche einen ATI Score von `r round(mean_ati_old,2)` haben. Daraus folgt, dass beide Gruppen ein ähnlich hohes Ausmaß an Technikaffinität aufzeigen.
...
ggplot(ati, aes(x = age, y = ati_mean)) +
...
ggplot(daten, aes(x = ati01, y = ati02)) +
  geom_point() +
  geom_smooth(method = "lm", color = "orange", se = FALSE, fullrange = TRUE)+
...
Insgesamt haben 125 Personen (*N =* 125) an der Online-Studie teilgenommen. Die dabei für die Analyse zu berücksichtigen Daten beinhalten die von 119 Teilnehmenden (*n =* 119). Zu den Personen, die herausgefiltert wurden, gehören eine Person, welche unter 18 Jahren ist und deswegen nicht berücksichtigt werden kann. Zudem 5 weitere Datensätze, die einer Testgruppe angehören. 
...
Wir betrachten in unserer Analyse der Daten die ATI-Skala, die HCT-Skala und die SUS-Skala.
...
Die 9-Item-Skala zur Affinität für Technologie-Interaktion (ATI) misst die Neigung einer Person, sich aktiv mit intensiver Technologie-Interaktion zu beschäftigen oder diese zu meiden, und wird als wichtige persönliche Ressource für den erfolgreichen Umgang mit Technologie betrachtet.
Bei unserer Studie liegt die durchschnittliche Technikaffinität der Teilnehmenden bei (*M =* 3.6, *SD =* 1.1). Die ATI-Skala hat einen möglichen Maximalwert von 6. Die Probanden besitzen dementsprechend weder eine besonders hohe, noch eine besonders niedrige Tendenz zur Technikaffinität. 

Um herauszufinden, wie sehr eine Person einem Computer oder einem System vertraut, wird die HCT-Skala (engl. Human Computer Trust) benutzt. Dies wird anhand der Indikatoren Kompetenz, wahrgenommene Zuverlässigkeit und der wahrgenommenen Verständlichkeit abgeleitet. Der Score wurde für die 2 unterschiedlichen Tools erfasst, wobei die Tools den Probanden in jeweils zufälliger Reihenfolge gezeigt wurden. HC1T hat dabei ein durchschnittliches Vertrauen von (*M =* 3.9, *SD =* 1) und HC2T ein durchschnittliches Vertrauen, welches bei (*M =* 4.1, *SD =* 0.9) liegt. Die HCT-Skala hat einen möglichen Maximalwert von 6. Insgesamt lässt sich daraus folgern, dass die Probanden beiden Systemen  gegenüber ein tendenziell höheres Vertrauen aufweisen, es aber leichte Abweichungen gibt, welche vermutlich auf die Zufälligkeit der Zuordnung und der unterschiedlichen Einschätzungen der beiden KI-Tools zurückzuführen sind.

Mithilfe der SUS-Skala kann man ermitteln, wie gebrauchstauglich ein System ist. Die Gebrauchstauglichkeit unserer KI-Systeme wurden durch die Teilnehmenden durchschnittlich mit (*M =* 3.9, *SD =* 0.6) bewertet. Im Allgemeinen lässt sich daraus erkennen, dass die Probanden mit der Gebrauchstauglichkeit der KI-Tools zufrieden sind, man aber aus dem Wert 5, welcher als höchstmöglicher Wert gilt, der vergeben werden kann, darauf schließen kann, dass es Raum für Verbesserung gibt.
...
#---------------------------------------------------------------
#Teil 1 Filtern und Data Wrangling


#appdatasummarize_unique1 <- ergebnisapp %>%
 # group_by(user_id) %>%
  #summarize(across(everything(), first))
#wegen diesem many to many relationship Ding 

#datengesamt <- gefilterte_daten %>%
 #full_join(appdatasummarize_unique1, by =  c("userId" = "user_id"))

#datengesamt <- datengesamt %>%
 #       select(starts_with("ATI")) %>%
...
ergebnissurvey <- ergebnissurvey %>% mutate(HCT_E = ifelse(first_variant == "eva-ai", keys_HCT1, keys_HCT2 ))
ergebnissurvey <- ergebnissurvey %>% mutate(HCT_E = ifelse(first_variant == "rd-ai", keys_HCT1, keys_HCT2 ))


deskriptive_statistik <- function(data, scale_name) {
  n <- nrow(ergebnissurvey)
  alpha <- psych::alpha(data %>% select(starts_with(scale_name)), check.keys = TRUE)$total$raw_alpha
  mean_val <- mean(data[[scale_name]], na.rm = TRUE)
  median_val <- median(data[[scale_name]], na.rm = TRUE)
  min_val <- min(data[[scale_name]], na.rm = TRUE)
  max_val <- max(data[[scale_name]], na.rm = TRUE)
  sd_val <- sd(data[[scale_name]], na.rm = TRUE)
...
#| message: false
#| label: tbl-skalen
#| tbl-cap: "Diskriptive Statistik für ATI, HCT und SUS"
#| tbl-cap-location: top
#| warning: false

#Tabelle für die verschiedenen Scores

ati_werte <- deskriptive_statistik(ergebnissurvey, "ATI")
hct1_werte <- deskriptive_statistik(ergebnissurvey, "HCT1T")
hct2_werte <- deskriptive_statistik(ergebnissurvey, "HCT2T")
sus_werte <- deskriptive_statistik(ergebnissurvey, "SUS")

tabelle_scores <- rbind(ati_werte, hct1_werte, hct2_werte, sus_werte)
row.names(tabelle_scores) <- c("ATI", "HCT1T", "HCT2T", "SUS")
...
### 4.5.1 Durchführung des t-tests 
Durchführung: 
Wir wollten untersuchen, ob es bei der Präzision der richtigen Entscheidungen altersbezogene signifikante Unterschiede gibt. 
Um die Unterschiede bezüglich des Alters zu vergleichen, haben wir zuerst einen Median-Split des Alters durchgeführt und unsere Daten in zwei Gruppen aufgeteilt: Eine Gruppe die jünger und eine die älter als der Median ist.
Nach dieser Aufteilung konnten wir den t-Test für beide KIs durchführen. Da wir zwei Variablen (Alter und Präzision) miteinander vergleichen wollen, haben wir einen paired t-test durchgeführt. 
...
data <- data.frame(
  ...
  mean_precision_eva_ai = c(mean(median_age_old$`precision_eva-ai`), mean(median_age_young$`precision_eva-ai`)),
  se_precision_eva_ai = c(sd(median_age_old$`precision_eva-ai`)/sqrt(length(median_age_old$`precision_eva-ai`)), sd(median_age_young$`precision_eva-ai`)/sqrt(length(median_age_young$`precision_eva-ai`))),
  mean_precision_rd_ai = c(mean(median_age_old$`precision_rd-ai`), mean(median_age_young$`precision_rd-ai`)),
  se_precision_rd_ai = c(sd(median_age_old$`precision_rd-ai`)/sqrt(length(median_age_old$`precision_rd-ai`)), sd(median_age_young$`precision_rd-ai`)/sqrt(length(median_age_young$`precision_rd-ai`)))
)

# Plot erstellen
ggplot(data, aes(x = group)) +
  geom_point(aes(y = mean_precision_eva_ai), color = "orange", size = 4) +
  geom_errorbar(aes(ymin = mean_precision_eva_ai - se_precision_eva_ai, ymax = mean_precision_eva_ai + se_precision_eva_ai), width = 0.2, color = "orange") +
  geom_point(aes(y = mean_precision_rd_ai), color = "green", size = 4) +
  geom_errorbar(aes(ymin = mean_precision_rd_ai - se_precision_rd_ai, ymax = mean_precision_rd_ai + se_precision_rd_ai), width = 0.2, color = "green") +
  labs(x = "Altersgruppe", y = "Mittelwert Präzision") +
  scale_x_discrete(labels = c("Younger" = "Jünger", "Older" = "Älter")) +  # Anpassung der x-Achsenbeschriftungen
  scale_color_manual(values = c("evaluative KI" = "orange", "Recommender KI" = "green"), labels = c("evaluative KI", "Recommender KI")) +  # Farben für die KIs
  theme_minimal()  # Anpassung des Themes, nach Bedarf

```
### 4.5.2 Auswertung des t-tests
Bei der Evaluierenden KI (@fig-ttest in orange) ergab der t-Test folgende Ergebnisse:
*t =* 0,92
*df =*  115.42
*p =* 0,36
...
Da es keinen signifikanten Unterschied in der Präzision der beiden Altersgruppen bei der evaluierenden KI gibt, ist nicht anzunehmen, dass das Alter einen Einfluss auf die Präzision beim Treffen von Entscheidungen hat.


Bei der Recommender-KI (@fig-ttest in grün) ergab der t-Test folgende Ergebnisse:
*t =* 1,42
*df =*  108.92
*p =* 0,16
...
### 4.5.3 Durchführung der Pearson-Korrelation 

Da wir uns in unserer Forschung besonders auf den Umgang verschiedener Altersgruppen mit den Systemen fokussieren, schauen wir uns den Zusammenhang zwischen dem Alter und dem HCT-Score, also dem Human-Computer-Trust genauer an. Um zu überprüfen, ob es hier tatsächlich eine relevanten Zusammenhang gibt, haben wir einen Pearson-Korrelationstest durchgeführt.
...
### 4.5.4 Auswertung der Pearson-Korrelation 

Die Analyse der Evaluierenden KI ergab einen signifikanten Zusammenhang mit einem t-Wert von -2,1155 und einem p-Wert von 0,03617. Dies deutet darauf hin, dass das Alter der Probanden signifikant negativ mit ihrem HCT-Score korreliert ist (*p <* 0,05). Der berechnete Korrelationskoeffizient von -0,1766103 zeigt an, dass ein höheres Alter tendenziell mit einem niedrigeren HCT-Score verbunden ist. 

Der Test zur Recommender KI liefert ebenfalls einen signifikanten Zusammenhang, mit einem t-Wert von -3,1246 und einem p-Wert von 0,002168. Auch hier war das Alter der Probanden signifikant negativ mit ihrem HCT-Score korreliert. Der Korrelationskoeffizient in diesem Fall betrug -0,2561782, was erneut darauf hinweist, dass ältere Probanden tendenziell niedrigere HCT-Scores aufwiesen. 

In beiden Analysen zeigte sich somit konsistent, dass das Alter der Probanden einen signifikanten Einfluss auf ihren HCT-Score hat, wobei ältere Probanden im Durchschnitt niedrigere HCT-Scores erreichten. 


### 4.5.5 Abschließende Zusammenfassung der Inferenzstatistik

In unserer inferenzstatistischen Analyse konnten wir herausstellen, dass ältere Probanden im Durchschnitt niedrigere HCT-Scores erreichten. Da das Vertrauen in die Systeme bei älteren Probanden niedriger ist, ließe sich vermuten, dass sich dies auch auf den Umgang und schließlich die Präzision der Einschätzung auswirkt. Hier konnten wir mit unserem t-Test allerdings keinen signifikanten Zusammenhang erkennen. Aktuell scheinen also sowohl ältere als auch jüngere Probanden unabhängig vom Vertrauen in das System die gleichen Ergebnisse zu erzielen. 
...
cor1 <- cor.test(ergebnissurvey$age, ergebnissurvey$HCT1T)
#cor1

ggplot(ergebnissurvey, aes(x = ergebnissurvey$age , y = ergebnissurvey$HCT1T)) +
  geom_point() +
...
#| message: false
#| warning: false
#| fig-cap: "Korrelation zwischen dem Alter der Probanden und dem HCT-Score der Recommender AI"
#| label: fig-cor2test

cor2 <- cor.test(ergebnissurvey$age, ergebnissurvey$HCT2T)
#cor2

ggplot(ergebnissurvey, aes(x = ergebnissurvey$age , y = ergebnissurvey$HCT2T)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE) +
  labs(title = "Korrelation zwischen Alter und HCT-Score der Eva-Ai",
       x = "Alter",
