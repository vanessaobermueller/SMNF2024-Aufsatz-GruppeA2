...
ergebnissurvey <- read.csv("/Users/vanessa/Documents/UNI/Semester II/Statisktik und Nutzerforschung/2024-06-11/surveydata.csv")

ergebnisapp <- read.csv("/Users/vanessa/Documents/UNI/Semester II/Statisktik und Nutzerforschung/2024-06-11/appdata.csv")
...
appdatasummarize <- ergebnisapp %>% group_by(user_id, variant) %>%
  summarize(mean_interactions = mean(number_of_interactions),
            precision = mean(correct_eval),
            first_variant = first(first_variant)) %>%
  pivot_wider(names_from = variant, values_from = c(mean_interactions, precision))

appdatasummarize_unique <- appdatasummarize %>%
  group_by(user_id) %>%
  summarize(across(everything(), first))

print(appdatasummarize_unique)

ergebnissurvey <- ergebnissurvey %>% left_join(appdatasummarize_unique, by = c("userId" = "user_id" ))

keys_ATI <- list(ATI = c("ATI.1.", "ATI.2.","-ATI.3.","ATI.4.","ATI.5.","-ATI.6.","ATI.7.","ATI.8.","-ATI.9."))


keys_HCT1 <- list(HCTT1 = c("HCT1T.01.","HCT1T.02.","HCT1T.03.","HCT1T.04.","HCT1T.05." ))
keys_HCT2 <- list(HCTT2 = c("HCT2T.01.","HCT2T.02.","HCT2T.03.","HCT2T.04.","HCT2T.05." ))

keys_SUS1 <- list(SUS1 = c("SUS.1.","SUS.2.", "SUS.3.", "SUS.4.", "SUS.5.", "SUS.6.", "SUS.7.", "SUS.8.", "SUS.9.", "SUS.10."))
keys_SUS2 <- list(SUS2 = c("SUS.1.","SUS.2.", "SUS.3.", "SUS.4.", "SUS.5.", "SUS.6.", "SUS.7.", "SUS.8.", "SUS.9.", "SUS.10."))


scored_ATI <- psych::scoreItems(keys_ATI, ergebnissurvey)
scored_HCT1 <- psych::scoreItems(keys_HCT1, ergebnissurvey)
scored_HCT2 <- psych::scoreItems(keys_HCT2, ergebnissurvey)
scored_SUS1 <- psych::scoreItems(keys_SUS1, ergebnissurvey)
scored_SUS2 <- psych::scoreItems(keys_SUS2, ergebnissurvey)

ergebnissurvey <- ergebnissurvey %>% cbind(scored_ATI$scores)
ergebnissurvey <- ergebnissurvey %>% cbind(scored_HCT1$scores)
ergebnissurvey <- ergebnissurvey %>% cbind(scored_HCT2$scores)
ergebnissurvey <- ergebnissurvey %>% cbind(scored_SUS1$scores)
ergebnissurvey <- ergebnissurvey %>% cbind(scored_SUS2$scores)

ergebnissurvey <- ergebnissurvey %>% mutate(HCT_E = ifelse(first_variant == "eva-ai", keys_HCT1, keys_HCT2 ))
ergebnissurvey <- ergebnissurvey %>% mutate(SUS_E = ifelse(first_variant == "eva-ai", keys_SUS1, keys_SUS2 ))



#print(names(ergebnissurvey))
#print(names(ergebnisapp))

gefilterte_daten <- ergebnissurvey %>%
  filter(age > 18 & age < 120 & groupId != "42")
#müssen noch danach filtern ob Leute den Fragebogen abgebrochen haben 


#datengesamt <- gefilterte_daten %>%
 # full_join(ergebnisapp, by =  c("userId" = "user_id"))
...
In unserer Fragenbogenstudie testest du ein KI Werkzeug (Künstliche Intelligenz), welches dir hilft irreführende Informationen im Internet zu erkennen.
